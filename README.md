<h1 align=center>Storeplate | AstroJs + Tailwind CSS + TypeScript Starter</h1>

<p align=center>A free, production-ready astro.js template powered by Tailwind CSS and TypeScript. This fork removes Shopify dependency and uses local JSON data for products.</p>
...
- 📦 Local products from JSON (no external API)
...
## 🛍️ Adding Your Own Products (No Shopify)

- Edit `src/data/products.json` and add items with fields:
  - `id`, `handle`, `title`, `description`, `vendor`, `tags`, `collections` (array of category names)
  - `images` (objects with `url`, `altText`, `width`, `height`)
  - `options` (e.g., Size, Color) and `variants` (each with `price.amount` and `currencyCode`)
  - Optional: `compareAtPrice` for showing a struck-through price
- Place product images under `public/images/` and reference them by URL (e.g., `/images/product-1.png`).
- Update home featured collection in `src/config/config.json` under `shopify.collections.featured_products` using the slugified name of one of your collections (e.g., `ropa`, `calzado`).
- Note: Cart and checkout are stubbed and do not process payments.
...
<!-- Shopify instructions removed: local JSON is used instead -->
...
- Shopify credentials are no longer required. `.env.example` is now empty.
