...
const CartModal: React.FC<Props> = ({ showTrigger = true }) => {
  ...
  if (!currentCart || currentCart.lines.length === 0 || quantity === 0) {
    return (
      <>
        ...
        <div
          id="cartOverlay"
          className={`fixed inset-0 bg-black opacity-50 z-20 transition-opacity ${isOpen ? "block" : "hidden"}`}
          onClick={closeCart}
        ></div>
                      ...
                      <a href={merchandiseUrl.toString()} className="z-30 flex flex-row space-x-4">
                        <div className="relative h-16 w-16 overflow-hidden rounded-md border border-neutral-300 bg-neutral-300">
                          <img
                            ...
                            src={(() => {
                              const color = item.merchandise.selectedOptions.find((option: any) => option.name === "Color")?.value;
                              const imgs = item.merchandise.product.images as any[];
                              const match = Array.isArray(imgs)
                                ? imgs.find((img: any) => img.altText === color)
                                : undefined;
                              return (
                                match?.url ||
